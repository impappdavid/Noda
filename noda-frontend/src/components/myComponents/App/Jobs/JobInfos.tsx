    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[450px] p-0 rounded-none border-none shadow-none overflow-hidden bg-white gap-0">
        ...
        {!submitted ? (
          ...
          /* SUCCESS STATE - MAINTAINS BOXED STYLE */
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide bg-white p-3">
        <ReactMarkdown
          ...
          components={{
            h2: ({ node, ...props }) => (
              <h2
                className="text-[12px] font-bold tracking-widest text-zinc-900 pb-1 mt-4 first:mt-0 "
                {...props}
              />
            ),
            h3: ({ node, ...props }) => (
              <h3
                className="text-[11px] font-bold tracking-wide text-zinc-800 mb-2 mt-4"
                {...props}
              />
            ),
            p: ({ node, ...props }) => (
              <p
                className="text-[12px] text-zinc-600 leading-relaxed mb-4 whitespace-pre-line"
                {...props}
              />
            ),
            ul: ({ node, ...props }) => (
            ...
            li: ({ node, ...props }) => (
              <li
                className="text-[12px] text-zinc-600 marker:text-zinc-500"
                {...props}
              />
            ),
            strong: ({ node, ...props }) => (
