...
import { SlidersHorizontal, Check, Target, ChevronRight, X } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
    ...
    <Dialog>
      ...
      <DialogContent className="max-w-xl bg-white border-none rounded-none p-0 gap-0 shadow-2xl overflow-hidden font-sans">
              ...
              {["recent", "relevance"].map((mode) => (
                <button
                  ...
                  className={cn(
                    ...
                    sortBy === mode
                      ? "bg-blue-500 text-white shadow-sm border border-zinc-300"
                  ...
                  <div
                    className={cn(
                      ...
                      selectedTypes.includes(type)
                        ? "bg-blue-500 border-blue-600 shadow-[0_0_8px_rgba(249,115,22,0.3)]"
            ...
            <div className="flex items-center gap-4 w-full">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-2 pr-6 pt-4 pb-1 border border-zinc-300 text-[12px] font-mono font-bold outline-none focus:border-blue-500 bg-zinc-50/50"
                  placeholder="0"
                />
              ...
              {skills.length > 0 ? (
                skills.map((skill) => (
                  <div
                    ...
                    className="flex items-center cursor-pointer gap-2 px-2 py-1 bg-blue-500 text-white text-[9px] font-bold uppercase tracking-widest transition-colors hover:bg-blue-600"
                  >
                    {skill}
                  </div>
                ))
              ) : (
                <span className="text-[9px] font-mono text-zinc-300 uppercase italic">
                  Buffer_Empty...
                </span>
              )}
            </div>
        ...
        <DialogFooter className="py-2 px-4 border-t border-zinc-300 flex items-center justify-between sm:justify-between w-full">
          ...
          <button className="rounded-none bg-zinc-900 hover:bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest h-10 px-12 transition-all shadow-lg cursor-pointer active:scale-95">
            Search
          </button>
        </DialogFooter>
      </DialogContent>
