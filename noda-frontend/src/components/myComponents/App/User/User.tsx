...
const UserProfile = () => {
    ...
    return (
        /* Global scroll enabled by removing h-screen and overflow-hidden */
                ...
                <main className="flex flex-1 border-x border-zinc-300 bg-white pt-13 mb-24 min-h-screen flex">
                    ...
                    <div className="flex-1 flex flex-col border-r border-zinc-300">
                        ...
                        <div className="relative shrink-0">
                            ...
                            <div className="px-3 relative pb-6 border-b border-zinc-300">
                                <div className="flex items-end justify-between -mt-10 mb-4">
                                    ...
                                    <button className="h-7 px-4 border border-zinc-300 text-[9px] font-black uppercase tracking-widest hover:bg-zinc-50 transition-all">Connect</button>
                                </div>
                        ...
                        <div className="flex w-full border-b border-zinc-300 bg-white sticky top-13 z-30 h-10 divide-x divide-zinc-200 shrink-0">
                            {['About', 'Posts'].map((tab) => (
                                <button key={tab} onClick={() => setActiveTab(tab)} className={cn("flex-1 text-[9px] font-mono font-black uppercase tracking-[0.2em] transition-colors", activeTab === tab ? "bg-zinc-900 text-white" : "text-zinc-500 hover:bg-zinc-50")}>
                                    {tab}
                                </button>
                            ))}
                        </div>
                        ...
                        <div className={cn("flex-1", activeTab === 'Posts' ? "p-0" : "p-6")}>
                            {activeTab === 'About' && (
                                <div className="space-y-12">
                                    ...
                                    <section className="space-y-4">
                                        <h4 className="text-[10px] font-mono font-black uppercase text-zinc-500 tracking-[0.2em]">04. Skills</h4>
                                        <div className="flex flex-wrap gap-1">
                                            {['Rust', 'Go', 'K8s', 'eBPF', 'Wasm'].map(skill => (
                                                <span key={skill} className="px-2 py-1 bg-zinc-800 text-white text-[10px] font-mono font-black uppercase">{skill}</span>
                                            ))}
                                        </div>
                                    </section>

                                    <section className="space-y-4">
                                        <h4 className="text-[10px] font-mono font-black uppercase text-zinc-500 tracking-[0.2em]">05. Certificates</h4>
                                        <div className="flex items-center gap-4 border border-zinc-100 p-3">
                                            <Award size={16} className="text-zinc-500" />
                                            <span className="text-[10px] font-bold uppercase">CKA: Certified Kubernetes Administrator</span>
                                        </div>
                                    </section>
                                </div>
                            )}
                    ...
                    {/* RIGHT: STATS SIDEBAR (w-40) - STICKY */}
...
const TimelineEntry = ({ role, org, date }: any) => (
    <div className="flex gap-4">
        <div className="w-11 h-11 bg-zinc-800 flex items-center justify-center shrink-0 border border-zinc-700 shadow-sm"><Briefcase size={16} className="text-white" /></div>
