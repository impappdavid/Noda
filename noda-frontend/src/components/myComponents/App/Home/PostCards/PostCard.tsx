    ...
    >
      ...
      <div className="flex justify-between items-start mb-1.5">
        <div className="flex gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5">
              <span className="text-xs font-bold text-zinc-900 hover:underline cursor-pointer truncate" onClick={(e) => onUserClick(e, post.author.username)}>
                {post.author.name}
              </span>
              <span className="text-[10px] font-mono text-zinc-400 truncate">@{post.author.username}</span>
            </div>
        ...
        <DropdownMenu modal={false}>
          <DropdownMenuTrigger asChild>
            <button className="text-zinc-400 hover:text-zinc-900 p-1 hover:bg-zinc-100 cursor-pointer transition-colors" onClick={(e) => e.preventDefault()}>
              <MoreHorizontal size={14} />
            </button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-32 rounded-none border-zinc-300 p-0 bg-white shadow-md" onClick={(e) => e.stopPropagation()}>
            ...
            <DropdownMenuItem className="gap-1.5 text-[10px] font-mono text-red-600 cursor-pointer py-1.5 rounded-none hover:bg-red-50/10">
              <Flag size={12} /> REPORT
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      ...
      <div className="pl-10">
        ...
        {post.images && post.images.length > 0 && (
          <div className={`overflow-hidden grid gap-0.5 mt-2 ${post.images.length === 1 ? "grid-cols-1" : "grid-cols-2"}`}>
            {post.images.slice(0, 4).map((img, idx) => (
              ...
              >
                <img src={img} className="w-full h-full object-cover" alt="Media content" />
