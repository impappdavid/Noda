...
const MOCK_PROJECT = {
  ...
  tagline:
    "Decentralized state synchronization protocol built for low-latency edge topology.",
  content:
    "Substituted heavy Raft consensus models with an asynchronous directed acyclic graph (DAG) ledger running over UDP/QUIC channels to optimize high-density edge clusters. This architecture eliminates master node bottlenecks entirely.",
  postedAgo: "1h",
  likes: 142,
  commentsCount: 56,
  views: "24.8k",
  ...
  challenges: [
    {
      title: "Challange 1",
      content:
        "Packet fragmentation over high-loss satellite hops required a completely customized rolling-window acknowledgment algorithm.",
    },
    {
      title: "Challange 2",
      content:
        "Zero-copy memory allocations inside the async network multiplexer initially triggered thread synchronization deadlocks during state updates.",
    },
  ],
  images: [
    ...
    <div className="min-h-screen text-zinc-900 font-sans flex flex-col relative">
      ...
      {selectedImg && (
        ...
        >
          ...
          <img
            src={selectedImg}
            className="max-w-full max-h-[85vh] object-contain rounded-none border border-zinc-700"
        ...
        <main className="flex flex-1 border-x border-zinc-300 bg-white min-h-screen flex-row pt-13">
          <div className="flex-1 flex flex-col mb-64">
            ...
            <div className="flex w-full items-center justify-between border-b border-zinc-300 bg-white sticky top-[52px] z-20 h-9  divide-x divide-zinc-300">
              ...
              <div className="flex-1 px-3 text-[9px] font-mono font-black text-zinc-400 uppercase tracking-[0.2em]">
                Project
              </div>
            ...
            <div className="flex-1 bg-zinc-300 gap-[1px] flex flex-col">
              ...
              >
                <div className="flex justify-between items-start">
                  <div className="flex gap-3 items-center">
                    <div className="w-10 h-10 rounded-none border border-zinc-300 overflow-hidden bg-zinc-50 shrink-0">
                      <img
                        src={MOCK_PROJECT.author.avatar}
                        alt=""
                        className="w-full h-full object-cover"
                      />
                    </div>
                    <div>
                      <span className="text-[11px] font-black text-zinc-900 block leading-tight hover:underline cursor-pointer">
                        {MOCK_PROJECT.author.name}
                      </span>
                      <span className="text-[10px] font-mono font-black text-zinc-400 block uppercase mt-0.5">
                        {MOCK_PROJECT.author.username} •{" "}
                        {MOCK_PROJECT.postedAgo} ago
                      </span>
                    </div>
                  </div>
                  <button
                    ...
                    className="p-2 text-zinc-500 hover:text-zinc-900 border border-transparent hover:bg-zinc-200 bg-white cursor-pointer rounded-none"
                  >
                    <Copy size={12} />
                  </button>
                </div>
                ...
                <div className="mt-4">
                  <h1 className="text-xl font-bold uppercase tracking-tighter text-zinc-900 leading-none">
                    {MOCK_PROJECT.title}
                  </h1>
                ...
                <div className="flex gap-2 text-[9px] font-mono text-zinc-600 mt-2 border-t border-zinc-100">
                  <div className="flex items-center gap-1 border border-zinc-300 px-1.5 py-0.5 bg-zinc-50">
                    <Calendar size={11} /> <strong>YEAR:</strong>{" "}
                    {MOCK_PROJECT.year}
                  </div>
                  <div className="flex items-center gap-1 border border-zinc-300 px-1.5 py-0.5 bg-zinc-50">
                    <Clock size={11} /> <strong>SPAN:</strong>{" "}
                    {MOCK_PROJECT.duration}
                  </div>
                </div>

                <p className="text-xs text-zinc-900 leading-relaxed mt-4">
                  ...
                  <div className="">
                    <div className="grid grid-cols-2 gap-0.5">
                      {MOCK_PROJECT.images.map((img, i) => (
                        <div
                          key={i}
                          className="bg-white  group cursor-pointer rounded-none"
                        >
              ...
              <div ref={sectionRefs.stack} className="bg-white">
                ...
                <div className="divide-x divide-zinc-300 flex flex-wrap ">
                  {MOCK_PROJECT.skills.map((skill, i) => (
                    <span
                      key={i}
                      className="text-[10px] font-mono bg-zinc-100 text-zinc-800 px-3 py-1.5  font-bold rounded-none"
                    >
                      {skill}
                    </span>
                  ))}
              ...
              <div ref={sectionRefs.friction} className="bg-white">
                ...
                <div className="grid grid-cols-2 divide-x divide-zinc-300">
                  {MOCK_PROJECT.challenges.map((challenge, i) => (
                    <div
                      key={i}
                      className="  last:border-0 hover:bg-zinc-100 p-2 items-start gap-3"
                    >
              ...
              <div ref={sectionRefs.endpoints} className="bg-white">
                <ModuleHeader
                  title="Gateway_Endpoints"
                  icon={<ExternalLink size={12} />}
                />
                <div className=" grid grid-cols-2 ">
                  <a
                    href={MOCK_PROJECT.links.github}
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center justify-between p-2 py-3 text-xs bg-white hover:bg-zinc-200 transition-colors text-zinc-800 font-mono font-bold rounded-none"
                  >
                   ...
                   <a
                    href={MOCK_PROJECT.links.live}
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center justify-between p-2 py-3 text-xs bg-blue-500 hover:bg-blue-600 transition-colors text-white font-mono font-bold rounded-none"
                  >
                    <span className="flex items-center gap-2">
                      Live Demo
                    </span>
                    <ArrowUpRight size={14} className="text-zinc-200" />
                  </a>
                </div>
              </div>
          ...
          <aside className="w-39 shrink-0 bg-white hidden md:flex flex-col border-l border-zinc-300">
            <div className="sticky top-13 flex flex-col h-[calc(100vh-3.5rem)]">
              ...
              <nav className="flex-1 overflow-y-auto border-b border-zinc-200">
                <NavBtn
                  label="Summary"
                  onClick={() => scrollTo(sectionRefs.identity)}
                />
                <NavBtn
                  label="Telemetry"
                  onClick={() => scrollTo(sectionRefs.telemetry)}
                />
                <NavBtn
                  label="System_Stack"
                  onClick={() => scrollTo(sectionRefs.stack)}
                />
                <NavBtn
                  label="Friction_Logs"
                  onClick={() => scrollTo(sectionRefs.friction)}
                />
                <NavBtn
                  label="Endpoints"
                  onClick={() => scrollTo(sectionRefs.endpoints)}
                />
              </nav>
            </div>
          </aside>
...
const ModuleHeader = ({ title, icon }: any) => (
  <div className="px-2 py-1.5 bg-zinc-300 border-b border-zinc-300 flex items-center gap-2 shadow-sm">
    ...
    <h3 className="text-[9px] font-mono font-black text-black uppercase tracking-[0.3em]">
      {title}
    </h3>
  </div>
);
...
const NavBtn = ({ label, onClick }: any) => (
  <button
    onClick={onClick}
    className="w-full text-left px-2 py-2 border-b border-zinc-300 text-[9px] font-mono font-bold text-zinc-500 uppercase hover:text-blue-600 cursor-pointer hover:bg-zinc-200 transition-all truncate rounded-none border-t-0 border-x-0 bg-transparent"
  >
    {label}
  </button>
);
  ...
  <div className="flex items-center justify-between text-zinc-500 font-mono text-[10px]">
    <div className="flex items-center gap-6">
      <button
        onClick={onLike}
        className={cn(
          "flex items-center gap-1.5 cursor-pointer transition-colors bg-transparent border-0 p-0 rounded-none",
          liked ? "text-orange-600 font-black" : "hover:text-zinc-900",
        )}
      >
        <Heart
          size={14}
      ...
      <div className="flex items-center gap-1.5">
        <BarChart3 size={14} /> {post.views}
      </div>
    </div>
    <button
      onClick={onBookmark}
      className={cn(
        "cursor-pointer transition-colors p-1 border border-transparent hover:border-zinc-300 bg-white rounded-none",
        bookmarked ? "text-zinc-900" : "hover:text-zinc-900",
      )}
    >
      <Bookmark
        size={14}
        className={bookmarked ? "fill-zinc-900 text-zinc-900" : ""}
      />
    </button>
  </div>
);
