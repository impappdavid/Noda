...
import { Search, Users, Inbox, Trash2, Clock, Send } from "lucide-react";
...
const NodeInjectionHeader = React.memo(
  ({ value, onChange, onTransmit }: any) => (
            ...
            {activeTab === "TEAM" ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
                {requests.map((node) => (
                  ...
                  >
                    {/* Top Section: Identity & Note */}
                    <div
                      onClick={() => onEdit(node)}
                      className="p-1.5 flex flex-col gap-2 cursor-pointer hover:bg-zinc-50/50 transition-colors"
                    >
                      <div className="flex items-center gap-2">
                        <div className="w-8 h-8 bg-zinc-900 flex items-center justify-center font-bold text-white text-[10px] border border-zinc-900 shrink-0 ">
                          {node.name
                      ...
                      {/* Condensed Note */}
                      <div className="bg-zinc-100/50 p-1.5 border-l-2 border-blue-500">
                        <p className="text-[10px] font-mono leading-tight text-zinc-700 line-clamp-2 italic">
                          "{node.note || "No protocol note attached..."}"
                        </p>
                      </div>
                    </div>

                    {/* Bottom Section: Integrated Action & Data Strip */}
                    <div className="flex border-t border-zinc-300 divide-x divide-zinc-300 bg-white h-10">
                      ...
                      <div className="flex-[1.5] px-2 flex flex-col justify-center bg-zinc-50">
                        ...
                        <span className="text-[9px] font-bold uppercase text-zinc-600 tracking-tighter truncate">
                          {node.role || "Frontend Developer"}
                        </span>
                      </div>

                      {/* Accept Button */}
                      <button
                        className="flex-1 text-[9px] font-black uppercase text-emerald-600 hover:bg-emerald-500 hover:text-white transition-all cursor-pointer outline-none"
                      >
                        Accept
                      </button>
                      ...
                      <button
                        className="flex-1 text-[9px] font-black uppercase text-zinc-400 hover:bg-red-500 hover:text-white transition-all cursor-pointer outline-none"
                      >
                        Ignore
                      </button>
                    </div>
