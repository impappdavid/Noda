...
import { 
    ...
    ShieldCheck, Mail, FileText, ExternalLink, 
    ArrowRight,
} from 'lucide-react';
...
const ReviewApplicantDialog = ({ app }: { app: ApplicantNode }) => {
    ...
    const requirements = ["React_Native", "Type_Safety", "System_Design", "Neural_Link_V1", "Encryption_Ops"];
    
    // Recruiter-critical data (Injected from Profile Components)
    const candidateSkills = ["React_Native", "Type_Safety", "System_Design", "Rust", "Go", "K8s", "TS", "eBPF"];
        ...
        <DialogContent className="sm:max-w-[650px] p-0 rounded-none border-none bg-white shadow-none overflow-hidden font-sans">
            ...
            <Tabs defaultValue="profile" className="w-full">
                <TabsList className="w-full h-10 grid grid-cols-2 rounded-none bg-zinc-100 p-0 gap-px border-b border-zinc-300">
                    <TabsTrigger value="profile" className="rounded-none text-[10px] font-mono font-black uppercase data-[state=active]:text-zinc-900 border-r border-zinc-200 cursor-pointer">
                        01_Candidate_Profile
                    </TabsTrigger>
                    <TabsTrigger value="cv" className="rounded-none text-[10px] font-mono font-black uppercase data-[state=active]:bg-white data-[state=active]:text-zinc-900 cursor-pointer">
                        02_Curriculum_Vitae
                    </TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="profile" className="m-0 p-4 space-y-4">
                    ...
                    <div className="flex justify-between items-start border-b border-zinc-100 pb-4">
                        <div className="space-y-1">
                            <h2 className="text-2xl font-bold tracking-tighter leading-none">{app.name}</h2>
                            <span className="text-[10px] font-mono font-bold text-zinc-500 tracking-widest flex items-center gap-2">
                                <Mail size={12} /> {app.name.toLowerCase().replace(" ", ".")}@noda.network
                            </span>
                        </div>
                        <div className="text-right space-y-1">
                            ...
                            <div className="flex items-center justify-end gap-1.5">
                                ...
                                <span className="text-[9px] font-mono font-black text-zinc-400 uppercase">Avail: Immediate</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="space-y-4">
                            <div className="p-2 border border-zinc-300 bg-zinc-50/30 space-y-3">
                                <span className="text-[9px] font-mono font-black text-zinc-500 uppercase tracking-widest block">Recruiter_Brief</span>
                                <div className="space-y-2">
                                    <div className="flex justify-between border-b border-zinc-300 pb-1">
                                        <span className="text-[9px] font-mono font-black text-zinc-500 uppercase">Total_Experience</span>
                                        <span className="text-[10px] font-bold text-zinc-900 uppercase">{experience}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-[9px] font-mono font-black text-zinc-500 uppercase">Education</span>
                                        <span className="text-[9px] font-bold text-zinc-900 uppercase text-right leading-tight max-w-[120px]">{education}</span>
                                    </div>
                                </div>
                            </div>

                            <div className="p-3 border border-zinc-200 space-y-3">
                                <span className="text-[8px] font-mono font-black text-zinc-500 uppercase tracking-widest block">Core_Matrix</span>
                                <div className="flex flex-wrap gap-1">
                                    {candidateSkills.map(skill => (
                                        <span key={skill} className="px-1.5 py-0.5 border border-zinc-300 text-zinc-600 text-[9px] font-mono font-black uppercase bg-white">
                                            {skill}
                                        </span>
                                    ))}
                                </div>
                        ...
                        <div className="space-y-4">
                            <div className="p-3 border border-zinc-900 bg-white space-y-3 h-full">
                                <span className="text-[9px] font-mono font-black text-zinc-500 uppercase tracking-widest block">Neural_Matching_Data</span>
                                <div className="flex flex-col gap-2">
                                    {requirements.map((req) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="flex items-center gap-2">
                                                    {isMatch ? <CheckCircle2 size={10} /> : <div className="w-2.5 h-2.5 rounded-full border border-zinc-300" />}
                                                    {req}
                                                </div>
                                                {isMatch && <span className="text-[9px] font-mono font-bold opacity-70">CONFIRMED</span>}
                                            </div>
                                        );
                                    })}
                                </div>
                    ...
                    <div className="pt-2 border-t border-zinc-300">
                        <span className="text-[9px] font-mono font-black text-zinc-500 uppercase tracking-widest block mb-3">Verification_Timeline</span>
                        <div className="grid grid-cols-2 gap-4  scale-95 origin-left">
                            <div className="flex gap-3 border-l-2 border-zinc-900 pl-3">
                                <div className="flex flex-col">
                                    <span className="text-[9px] font-mono font-black text-zinc-500 uppercase">2024 - PRES</span>
                                    <h4 className="text-[11px] font-bold uppercase">Lead Engineer</h4>
                                    <span className="text-[10px] font-mono font-black text-zinc-500 uppercase">Noda Labs</span>
                                </div>
                            </div>
                            <div className="flex gap-3 border-l-2 border-zinc-300 pl-3">
                                <div className="flex flex-col">
                                    <span className="text-[9px] font-mono font-black text-zinc-500 uppercase">2021 - 2024</span>
                                    <h4 className="text-[11px] font-bold uppercase">Senior Dev</h4>
                                    <span className="text-[10px] font-mono font-black text-zinc-500 uppercase">Vercel</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <TabsContent value="cv" className="m-0 h-[450px] bg-zinc-200 relative">
                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
                        ...
                        <span className="text-[10px] font-mono font-black text-zinc-500 uppercase tracking-widest">PDF_Uplink_Preview</span>
...
const AcceptApplicantDialog = () => (
    <DialogContent className="sm:max-w-[400px] p-0 rounded-none border-none bg-white shadow-none font-sans overflow-hidden">
        ...
        <div className="p-4 text-center space-y-4">
            ...
            <div className="space-y-2">
                <h3 className="text-sm font-black uppercase tracking-tighter italic text-zinc-900">Node_Acceptance_Successful</h3>
            ...
            <div className="flex flex-col gap-2 pt-1">
                ...
                <div className="group">
                    <button className="w-full h-12 border border-red-300 text-red-600 text-[10px] font-mono font-black uppercase hover:bg-red-50 transition-all cursor-pointer flex flex-col items-center justify-center leading-none">
                        ...
                        <span className="text-[9px] opacity-60 mt-1 font-bold tracking-tighter">Reject_Remaining_Applicants</span>
                    </button>
                    <div className="hidden group-hover:flex absolute top-12 left-0 right-0 bg-red-600 text-[12px] text-white p-2 font-mono uppercase tracking-tighter items-center gap-2 border border-red-700 shadow-lg animate-in fade-in zoom-in-95 duration-300 z-50">
                        <span>Warning: Critical action. This will send rejection signals to all other nodes in this job queue.</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div className="p-2 border-b border-zinc-300 bg-white hover:bg-zinc-50 transition-all group flex items-center justify-between border-b border-zinc-100">
        ...
        <div className="flex items-center gap-4 ">
            <div className="flex flex-col items-end mr-4">
                <span className="text-[8px] font-mono font-black text-zinc-500 uppercase mb-1 leading-none tracking-tighter">Response_Window</span>
                <div className={cn(
                    "px-2 py-1 border text-[9px] font-mono font-black uppercase",
                    app.deadline.includes('13d') ? "border-emerald-200 bg-emerald-50 text-emerald-600" : "border-orange-200 bg-orange-50 text-orange-600"
                )}>
                    {app.deadline}
                </div>
            </div>
