    ...
    <div className="bg-white">
      <Dialog>
        <DialogTrigger asChild>
          <button className="w-full flex items-center justify-between bg-zinc-800 px-3 py-3 text-sm text-zinc-300 hover:bg-zinc-900 transition-all text-left cursor-pointer group">
        ...
        <DialogContent className="sm:max-w-xl max-h-[90vh] flex flex-col rounded-none p-0 overflow-hidden bg-white border-none shadow-none">
          {/* Header */}
          <DialogHeader className="bg-zinc-200 p-2 text-black flex flex-row items-center justify-between space-y-0 shrink-0">
            ...
            <DialogClose asChild>
              <button className="bg-transparent p-1 hover:bg-zinc-300 text-zinc-500 hover:text-black transition-colors cursor-pointer">
          ...
          <div className="flex-1 px-2 mt-2 space-y-2 overflow-y-auto scrollbar-hide">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <div className={`w-10 h-10 flex items-center justify-center transition-all duration-200 overflow-hidden ${
                  isAnonymous ? "bg-zinc-300" : "bg-zinc-100"
                }`}>
                  {isAnonymous ? (
                ...
                <div className="flex flex-col text-left">
                  <div className="flex gap-2 items-end">
                    <span className={`text-xs font-mono font-black tracking-tight uppercase transition-colors ${
                    ...
                    <span className="text-[10px] font-mono tracking-tight text-zinc-500">
                      {isAnonymous ? "@encrypted" : "@alexrivers"}
                    </span>
                  </div>
              ...
              <button
                ...
                className={`p-2 flex items-center justify-center border transition-all duration-150 active:scale-90 cursor-pointer ${
                  isAnonymous
            ...
            {images.length > 0 && (
              <div className={`grid gap-1 border border-zinc-200 overflow-hidden mb-4 ${
                images.length === 1 ? "grid-cols-1" : "grid-cols-2"
              }`}>
                {images.map((src, idx) => (
                  <div key={idx} className="relative bg-zinc-100 aspect-auto">
            ...
            {showPoll && (
              <div className="mt-2 mb-4 bg-white border border-zinc-300 animate-in slide-in-from-top-2">
                ...
                {pollOptions.length < 4 && (
                  <button
                    type="button"
                    onClick={addOption}
                    className="w-full h-8 flex items-center justify-center gap-2 border-t border-zinc-300 bg-white text-[9px] font-mono font-black text-zinc-500 uppercase hover:bg-blue-500 hover:text-white transition-all cursor-pointer"
                  >
            ...
            <div className="flex items-center gap-1 py-3 border-t border-zinc-200">
              ...
              <button
                ...
                className={`p-2 flex gap-1 text-sm transition-colors cursor-pointer hover:bg-zinc-300 ${
                  images.length >= 4 ? "text-zinc-200 pointer-events-none" : "text-zinc-500 hover:text-zinc-900"
                }`}
                disabled={images.length >= 4}
              >
                <ImageIcon size={18} />
              ...
              <button
                ...
                className={`p-2 flex gap-1 text-sm transition-colors cursor-pointer hover:bg-zinc-300 ${
                  showPoll ? "text-blue-500" : "text-zinc-500 hover:text-zinc-900"
                }`}
              >
                <BarChart2 size={18} />
                Poll
              ...
              <button className="ml-auto bg-blue-500 hover:bg-blue-600 text-white text-[10px] rounded-none h-10 px-8 font-mono font-black uppercase tracking-widest transition-colors cursor-pointer">
                Post
              </button>
            </div>
          </div>
