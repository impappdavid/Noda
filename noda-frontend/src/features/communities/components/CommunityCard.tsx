...
import { ArrowRight, Users, Globe, Hash } from "lucide-react";
...
const CommunityCard: React.FC<CardProps> = ({ community }) => {
  ...
  return (
    ...
    >
      ...
      <div className="p-2 pb-1 flex items-center gap-2">
        {/* LOGO BOX */}
        <div className="w-10 h-10 bg-zinc-300 flex items-center justify-center  shrink-0 ">
          {community.logo ? (
            <img
              src={community.logo}
              alt={community.name}
      ...
      <div className="px-2 py-2 flex flex-col gap-2">
        <p className="text-[11px] text-zinc-900 font-mono leading-[1.3] uppercase line-clamp-2 h-fit">
          Operationalizing secure intelligence protocols for the{" "}
          {community.name} network environment.
        </p>

        <div className="flex items-center justify-between border-t border-zinc-300 pt-2">
          <div className="flex items-center gap-1.5">
            <Users size={10} className="text-blue-500" />
            <span className="text-[10px] font-mono font-bold text-zinc-900  uppercase tracking-tighter">
              {community.members}{" "}
              <span className="text-[9px] text-zinc-600">user</span>
            </span>
          </div>
        </div>
      </div>
